...
namespace GGD_Hack.Features
{
    ...
    public class AverageKillButtonToKillButton : MonoBehaviour
    {
        ...
        //2.20.01
        //修改玩家从服务器收到的角色身份
        //48 89 5C 24 20 56 48 83 EC 20 80 3D ?? ?? ?? ?? ?? 48 8B DA 48 8B F1 0F 85 A3 00 00 00 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? C6 05 ?? ?? ?? ?? ?? 48 89 6C 24 30 48 89 7C 24 38 4C 89 74 24 40 48 89 5E 18 48 85 DB 0F 84 80 02 00 00 83 7B 18 00
        [HarmonyPatch(typeof(Managers.GameManagers.EventsManagers.PluginEventsManager), nameof(Managers.GameManagers.EventsManagers.PluginEventsManager.IMFOMCKGKAJ))]
        ...
        {
            static void Prefix(SpawnedPlayerHandler __instance, ref AEMEBPDOAPE __0)
            {
                if (!Enabled.Value) return;
                if (!__instance.playerController.isLocal) { return; }

                if ((int)__instance.playerController.playerRole.BELBKBCFPIL == (int)GameData.RoleId.Avenger)
                {
                    __0 = AEMEBPDOAPE.Sheriff;
                    isAvenger = true;
                }
                else
                {
